-- Liefert Pending, Dead (24h), Avg Attempts (7d), Delivered (24h) und Top-Errors (24h)
...
begin
  ...
  from integration_outbox
  where status = 'dead' and created_at >= v_since_24h;

  select count(*) into v_delivered24
  from integration_outbox
  where status = 'delivered' and delivered_at >= v_since_24h;

  select coalesce(avg(attempts)::numeric(10,2),0) into v_avg_attempts7
  from integration_outbox
  where status = 'delivered' and delivered_at >= v_since_7d;

  select coalesce(jsonb_agg(jsonb_build_object(
           'error', left(coalesce(last_error,''),160),
           'cnt', cnt
